...
class Technology:
    ...
    def get_lcoe(self, energy_per_cell, people, num_people_per_hh, start_year, end_year, new_connections,
                 ...
                 get_investment_cost_transformer=False, get_investment_cost_connection=False,get_invesment_transmission=False):
        """
        ...
        elif get_invesment_transmission:
...
class SettlementProcessor:
    ...
    def __init__(self, path):
        try:
            self.df = pd.read_csv(path, skip_blank_lines=True)
    ...
    def pre_electrification(self, grid_calc, grid_price, year, time_step, start_year):
        ...
        self.df['MG_Hydro2025'] = 100
        self.df['MG_PV2025'] = 100
        self.df['MG_Wind2025'] = 100
        self.df['MG_Diesel2025'] = 100
        self.df['SA_Diesel2025'] = 100
        self.df['SA_PV2025'] = 100
        ...
        # grig_init is 1 then LCOEof the grid is chosen
        self.df.loc[
    ...
    def elec_extension(self, grid_calc, max_dist, year, start_year, end_year, timestep, grid_cap_gen_limit):
        ...
        for unelec in unelectrified:
            grid_lcoe = grid_calc.get_lcoe(energy_per_cell=enerperhh[unelec],
                                           ...
                                           elec_loop=0)
#            comida2.loc[unelec,'LCOE'] = grid_lcoe
            if grid_lcoe < min_code_lcoes[unelec]:
...
#                print(sum(self.df[self.df[SET_LIMIT + "{}".format(year)] == 1][SET_POP + "{}".format(year)]))
#                print(self.df[SET_POP + "{}".format(year)].sum())
#                print( elecrate)
    ...
    def Demand_Regressions(self, *args):
              
                        
         
         self.df['HouseHolds'] = (self.df['Pop2025']/self.df['NumPeoplePerHH'])  
         
    
         for i in self.df.index:
             
             for j in args:
                if j.altitude[0] < self.df['Elevation'][i] <  j.altitude[1]:
                    if j.latitude[0] < self.df['Y_deg'][i] <  j.latitude[1]:                        
                        self.df.loc[i,'Location'] = j.name
             path = 'Surrogate_Models/Demand/demand_regression_' + self.df.loc[i,'Location'] + '.joblib'
             demand = load(path)    
             HouseHolds = np.array(self.df.loc[i,'HouseHolds'])
             HouseHolds = HouseHolds.reshape(-1, 1)
             Demand = demand.predict(HouseHolds)
             Demand = Demand[0][0]
             self.df.loc[i,'ExpectedDemand'] = Demand/(1 - 0.05) 


    def Surrogate_Hybrid(self, discount_rate, years, *args):
        years = 20
        a = discount_rate*((1+discount_rate)**years)
        b = ((1 + discount_rate)**years)-1
        Capital_Recovery_Factor = a/b
        
        Technology.set_default_values(base_year=2012,
                                      start_year=2012,
                                      end_year=2025,
                                      discount_rate=0.12)
        
        distribution_grid_calc = Technology(om_of_td_lines=0.02,
        ...
        for i in self.df.index:
            ...
            for j in range(len(args)):
                if args[j].name == self.df.loc[i,'Location']:
                    loc = args[j]
            if  40  <= self.df['HouseHolds'][i] < 570:
                
                path = 'Surrogate_Models/Hybrid/NPC_' + self.df.loc[i,'Location'] + '.joblib'
                NPC = load(path)
                path = 'Surrogate_Models/Hybrid/PV_' + self.df.loc[i,'Location'] + '.joblib'
                PV = load(path)
                
                fuel_cost = (loc.fuel_cost + 2 * loc.fuel_cost * loc.diesel_truck_consumption * 
                             (self.df['TravelHours'][i])/loc.diesel_truck_volume)  # LCOE 
               
                X = pd.DataFrame()
                X.loc[1, 'Renewable Invesment Cost'] = loc.renewable_invesment_cost  
                X.loc[1, 'Battery Invesment Cost'] =   loc.battery_invesment_cost
                X.loc[1, 'Genset Invesment Cost'] = loc.genset_invesment_cost  
                X.loc[1, 'Fuel Cost'] = fuel_cost 
                X.loc[1, 'HouseHolds'] = self.df['HouseHolds'][i]  
                X.loc[1, 'GHI'] = 440 
                
                self.df.loc[i,'Fuel_Cost'] = fuel_cost 
                self.df.loc[i,'NPC_Hybrid'] = NPC.predict(X)[0][0] 
                
                self.df.loc[i,'NPC_Distribution_Hybrid'] = distribution_grid_calc.get_lcoe(energy_per_cell = self.df['EnergyPerSettlement2025'][i],
                                                                     people = self.df['Pop2025'][i], 
                                                                     num_people_per_hh =  self.df['NumPeoplePerHH'][i], 
                                                                     start_year=2012, 
                                                                     end_year=2025,  
                                                                     new_connections= self.df['NewConnections2025'][i], 
                                                                     total_energy_per_cell= self.df['TotalEnergyPerCell'][i],
                                                                     prev_code= self.df['FinalElecCode2012'][i],
                                                                     grid_cell_area=self.df['GridCellArea'][i],
                                                                     get_invesment_transmission=True)
                
                self.df.loc[i,'NPC_Total_Hybrid'] = (self.df.loc[i,'NPC_Distribution_Hybrid'] +
                                                       self.df.loc[i,'NPC_Hybrid'])
                self.df.loc[i, 'Actualize_Demand'] = self.df['ExpectedDemand'][i]/Capital_Recovery_Factor
                self.df.loc[i, 'LCOE Hybrid'] = self.df.loc[i,'NPC_Total_Hybrid']/self.df.loc[i, 'Actualize_Demand']
                self.df.loc[i,'PV_Hybrid'] = PV.predict(X)[0][0] 
                
            else:
                self.df.loc[i,'NPC_Hybrid'] = 99
                self.df.loc[i,'Fuel_Cost'] =  99
                self.df.loc[i,'NPC_Hybrid'] = 99
                self.df.loc[i,'NPC_Distribution_Hybrid'] = 99
                self.df.loc[i,'NPC_Total_Hybrid'] = 99
                self.df.loc[i, 'Actualize_Demand'] = 99
                self.df.loc[i, 'LCOE Hybrid'] = 99
                self.df.loc[i,'PV_Hybrid'] = 99                
                
                
    def Surrugate_HouseHold(self, *args):
        ...
        for i in self.df.index:
            ...
            path = 'Surrogate_Models/HouseHolds/NPC_H_' + self.df.loc[i,'Location'] + '.joblib'
            NPC = load(path)
            
            path = 'Surrogate_Models/HouseHolds/LCOE_H_' + self.df.loc[i,'Location'] + '.joblib'
            LCOE = load(path)
            
            path = 'Surrogate_Models/HouseHolds/PV_H_' + self.df.loc[i,'Location'] + '.joblib'
            PV = load(path)
            if  40  > self.df['HouseHolds'][i]:
                
                X = pd.DataFrame()
                X.loc[1, 'Renewable Invesment Cost'] = loc.renewable_invesment_cost_h  
                X.loc[1, 'Battery Invesment Cost'] =   loc.battery_invesment_cost_h            
                X.loc[1, 'GHI'] = 440
            
                self.df.loc[i,'NPC_HomeSystem'] = NPC.predict(X)[0][0]*self.df['HouseHolds'][i] 
                self.df.loc[i,'LCOE_HomeSystem'] = LCOE.predict(X)[0][0] 
                self.df.loc[i,'PV_HomeSystem'] = PV.predict(X)[0][0]*self.df['HouseHolds'][i]  
            
            else: 
            
                self.df.loc[i,'NPC_HomeSystem'] = 99 
                self.df.loc[i,'LCOE_HomeSystem'] = 99 
                self.df.loc[i,'PV_HomeSystem'] = 99
...
class location:        
    ...
    def __init__(self,
                 ...
                 diesel_truck_volume = 15000.0,
                 renewable_invesment_cost = 1250,
                 battery_invesment_cost = 500,
                 genset_invesment_cost = 1480,
                 fuel_cost = 0.53,
                 renewable_invesment_cost_h = 1500,
                 battery_invesment_cost_h = 700):  
        ...
        self.diesel_truck_consumption = diesel_truck_consumption
        self.diesel_truck_volume = diesel_truck_volume
        self.renewable_invesment_cost = renewable_invesment_cost
        self.battery_invesment_cost = battery_invesment_cost
        self.genset_invesment_cost = genset_invesment_cost
        self.fuel_cost = fuel_cost
        self.renewable_invesment_cost_h = renewable_invesment_cost_h
        self.battery_invesment_cost_h = battery_invesment_cost_h
